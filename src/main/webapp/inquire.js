...
function fetchAllInquires() {
  const url = "http://localhost:8080/ABCRestaurant/resources/inquiry";
...
async function AddInquiry() {
  ...
  const inquiry = {
    ...
    inquireType: document.getElementById("dpaddlInquireType").value,
    inquireDescription: document.getElementById("addtxtDescription").value,
    inquireFeedback: document.getElementById("addtxtFeedback").value,
    inquireReply: document.getElementById("dpaddlReply").value,
  };
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (userType == "Manager") {
        window.location.href = "managerInquire.html";
      }
    } else {
      Swal.fire({
        icon: "error",
        title: "Unable to add Inquiry. Please try again later.",
      });
    }
  } catch (error) {
    Swal.fire({
...
async function deleteInquiry() {
  const url =
    "http://localhost:8080/ABCRestaurant/resources/inquiry/deleteinquiry/";
  ...
  try {
    ...
    if (response.ok) {
      ...
      Swal.fire({
        icon: "error",
        title: "Unable to delete inquiry. Please try again later.",
      });
    }
  } catch (error) {
    Swal.fire({
